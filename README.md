...
**A game that plays differently every time.** The original Red Alert campaigns ship complete and playable in their classic form — 14 missions per side (Allied and Soviet), in order, just as you remember. But the **Enhanced Edition** transforms them into something the originals never were: a strategic campaign where the missions are the skeleton and *your choices are the muscle*. Westwood were already heading here — Tiberian Sun's world map let you choose which territory to attack, Red Alert 2 let you pick which country to invade — but scope constrained them from going further. Two decades of games like XCOM, Total War, and Into the Breach have since proven that a lean strategic layer between tactical missions works brilliantly. The Enhanced Edition completes what Westwood started. Between milestone missions, the War Table presents operations, enemy initiatives, and an arms race that shapes every battle ahead. Capture a Chrono Tank prototype from Italy or deny the Soviets their super-soldier program. Every tech, every weapon, every enemy capability has an acquisition path, a denial path, and a commander-choice trade-off. The final mission is different every playthrough — different units, different enemy composition, different approach routes, different briefing text — because it is the culmination of every decision you made. The classic 14-mission path is always available for purists. The Enhanced Edition is for players who want to command a war, not just play missions. Switchable balance presets, AI personalities, and pathfinding models — per lobby, not per mod. "Train Against" AI that mimics real players from their replays. Weather systems that change terrain.
...
- **Two ways to play every campaign.** The original 14 missions per side ship in their classic form — linear, faithful, complete. The **Enhanced Edition** wraps those same missions in a strategic layer inspired by XCOM, Total War, and Jagged Alliance: a War Table where you choose operations, counter enemy initiatives, and build an arsenal that shapes every mission ahead
- **The War Table.** Between milestone missions, the player commands the war — not just the battles. Pick which operations to run. Watch Soviet initiatives advance. Build your tech through side missions. Every choice is an opportunity cost: run the Italy chain for a Chrono Tank, or the Poland chain for Super Tanks? Deny the enemy's chemical weapons, or focus on your own air superiority? You can't do everything. That tension is the game
- **A dynamic arms race.** Expansion-pack units are not free unlocks — they are campaign rewards. Capture prototypes, deny enemy programs, and shape the battlefield through operations. The commando path yields full-quality tech; the commander path trades precision for broader denial. Three-state outcomes (acquired / partial / denied) mean every technology tells a story. The final mission is different every playthrough because it reflects every choice you made
- **Classic campaign always available.** The original 14 missions per side ship as a separate, faithful reproduction — same missions, same difficulty, same experience. No War Table, no operations, no strategic layer. For players who want the 1996 campaign exactly as it was. Within the Enhanced Edition, players who skip optional operations face a harder road — no bonus assets, no denied enemy tech — but the campaign remains completable. Two different choices: play the original, or play the Enhanced Edition your way
- **Continuous flow** — briefing, mission, debrief, next mission. No exit-to-menu between levels. Defeat continues the story
- **Persistent state** — surviving units, veterancy, captured equipment, hero progression, and campaign flags carry forward between missions
...
- **"One more operation" momentum** — the War Table's Requisition and Command Authority systems create a Civ-like "one more turn" loop: run an operation, see new intel, spot an expiring opportunity, run one more before the main mission hits. The pacing is tuned so the player always wants to do slightly more than their resources allow
