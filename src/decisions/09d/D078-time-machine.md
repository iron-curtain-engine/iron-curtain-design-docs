...
| **C&C Red Alert** (1996) | Chronosphere teleports units. Einstein's time travel created the RA timeline. Chrono Vortex hazard from overuse | The lore *is* time travel. Chronosphere is iconic. Overuse penalties (vortex) create tension | IC owns this lore. Frame the time machine as "what if the Chronosphere experiments went further?" |
...
| **Dark** (Netflix) — Bootstrap Paradox | Objects exist in causal loops with no origin. The time machine's blueprints are given to the person who builds the time machine | **Self-causing technology.** A campaign mission where the player captures enemy Chronosphere technology that turns out to be the prototype *they* sent back in time. The player's own time-travel journey created the enemy's weapon — deeply fitting for C&C's Einstein narrative |
...
campaign:
  ...
  time_machine:
    ...
    narrative_name: "Chronosphere Mark II" # displayed in UI and briefings
...
- **Allied campaign:** Einstein's Chronosphere experiments produce an unexpected side effect — limited temporal communication. The Allies can send tactical intel back in time, but each transmission destabilizes the device
